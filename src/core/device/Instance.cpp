...
{
    uint32_t layerCount;
    vulkan::EnumerateInstanceLayerProperties(&layerCount, nullptr);

    std::vector<VkLayerProperties> availableLayers(layerCount);
    vulkan::EnumerateInstanceLayerProperties(&layerCount, availableLayers.data());
...
auto check_required_extensions(const std::vector<const char*>& extensions) -> bool
{
    ...
    vulkan::EnumerateInstanceExtensionProperties(nullptr, &extensionCount, nullptr);

    std::vector<VkExtensionProperties> availableExtensions(extensionCount);
    vulkan::EnumerateInstanceExtensionProperties(nullptr, &extensionCount, availableExtensions.data());
    ...
    m_validationLayers{layers}
{
    ...
    vulkan::CreateInstance(&createInfo, nullptr, &m_instance);

    if (common::DEBUG) {
        const auto create_info = vulkan::get_debug_messenger_create_info();

        vulkan::CreateDebugUtilsMessengerEXT(
            m_instance,
            &create_info,
            nullptr,
            &m_debugMessenger
...
Instance::~Instance()
{
    ...
    if (m_instance != VK_NULL_HANDLE) {
        vulkan::DestroyInstance(m_instance, nullptr);

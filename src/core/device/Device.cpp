...
auto get_physical_device(
    const core::device::Instance& instance,
    [[maybe_unused]] const core::device::Surface& surface
) -> VkPhysicalDevice
{
    uint32_t deviceCount = 0;
    vulkan::EnumeratePhysicalDevices(instance.getInstance(), &deviceCount, nullptr);
    ...
    vulkan::EnumeratePhysicalDevices(instance.getInstance(), &deviceCount, devices.data());
    ...
    for (const auto& device : devices) {
        ...
        vulkan::GetPhysicalDeviceMemoryProperties(device, &memoryProps);
...
auto get_queue_families(
    ...
    vulkan::GetPhysicalDeviceQueueFamilyProperties(physDevice, &queueFamilyCount, nullptr);

    std::vector<VkQueueFamilyProperties> queueFamilies(queueFamilyCount);
    vulkan::GetPhysicalDeviceQueueFamilyProperties(physDevice, &queueFamilyCount, queueFamilies.data());
    ...
    for (uint32_t i = 0; i < queueFamilyCount; i++) {
        ...
        vulkan::GetPhysicalDeviceSurfaceSupportKHR(physDevice, i, surface, &presentSupport);
...
{
    if (m_graphicsQueue) {
        vulkan::QueueWaitIdle(m_graphicsQueue.queue);
        m_graphicsQueue = {};
    }

    if (m_presentQueue) {
        vulkan::QueueWaitIdle(m_presentQueue.queue);
        m_presentQueue = {};
    }

    if (m_device != VK_NULL_HANDLE) {
        vulkan::DestroyDevice(m_device, nullptr);

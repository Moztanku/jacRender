...
class Semaphore {
public:
    Semaphore(device::Device& device) : m_device{device.getDevice()} {
    ...
    ~Semaphore() {
        if (m_semaphore != VK_NULL_HANDLE) {
            vulkan::DestroySemaphore(m_device, m_semaphore, nullptr);
...
class Fence {
public:
    Fence(device::Device& device, bool signaled = true) : m_device{device.getDevice()} {
    ...
    ~Fence() {
        if (m_fence != VK_NULL_HANDLE) {
            vulkan::DestroyFence(m_device, m_fence, nullptr);
    ...
    auto wait(const uint64_t timeout = UINT64_MAX) const -> VkResult {
        return vulkan::WaitForFences(m_device, 1, &m_fence, VK_TRUE, timeout);
    }

    auto reset() const -> VkResult {
        return vulkan::ResetFences(m_device, 1, &m_fence);
    }

    auto isSignaled() const -> bool {
        return vulkan::GetFenceStatus(m_device, m_fence) == VK_SUCCESS;
    }
